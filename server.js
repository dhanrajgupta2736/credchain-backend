...
app.post('/fetch-result', async (req, res) => {
  ...
  try {
    ...
    // Pick the table with the MOST valid subject rows to avoid duplicates
    let bestRows = [];
    $('table').each((i, table) => {
      ...
      if (headerText.includes('course') || (headerText.includes('sem') && headerText.includes('grade'))) {
        ...
        $(table).find('tr').each((j, row) => {
          ...
          if (cells.length >= 6) {
            ...
            // Only real subject rows have a proper course code like NYCT301
            if (code && name && /^[A-Z]{2,}\d/.test(code)) {
              candidateRows.push({ sem, code, name, type, credits, grade, revalGrade, result });
            }
          }
        });
        if (candidateRows.length > bestRows.length) bestRows = candidateRows;
      }
    });

    // Deduplicate by course code (keep first occurrence only)
    const seenCodes = new Set();
    for (const row of bestRows) {
      if (!seenCodes.has(row.code)) {
        seenCodes.add(row.code);
        if (!semester && row.sem) semester = row.sem;
        subjects.push(row);
      }
    }
